...
class PlayerPt2:
    ...
    def rank_hand(self):
        ...
        for unique_card in unique_cards:
            cards_count[unique_card] = self.hand.count(unique_card)
        # The lower, the stronger
        # Give J to highest freq
        # If it contains a J and isn't all J
        if cards_count.get("J") and len(cards_count.keys()) > 1:
            num_j = cards_count.pop("J")
            cards_count[max(cards_count, key=cards_count.get)] += num_j


        unique_cards = list(set(cards_count.keys()))
        ordered_values = sorted(list(cards_count.values()))
...
for line in txt.split("\n"):
    hand, bid = line.split(" ")
    players.append(PlayerPt2(hand, int(bid)))
